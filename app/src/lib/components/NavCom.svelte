...
<header class="absolute inset-x-0 top-0 z-50">
	<nav class="flex items-center justify-between p-6 lg:px-8" aria-label="Global">
		...
		<div class="hidden lg:flex lg:gap-x-12">
			{#each menu.home as item}
				<a href={item.url} class="text-sm font-semibold leading-6 text-dark">{item.name}</a>
			{/each}
		</div>
		<div class="hidden lg:flex lg:flex-1 lg:justify-end">
			<a href="/login" class="text-sm font-semibold leading-6 text-dark"
				>Log in or Register <span aria-hidden="true">&rarr;</span></a
			>
		</div>
	</nav>
